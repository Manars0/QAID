    ...
    <section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
        {items.map((item: any, i: number) => {
          ...
          // One-line reason: first sentence, max 80 chars
          const src = item.details || item.requirement || ""
          const dot = src.indexOf(".")
          const raw = dot > 0 ? src.slice(0, dot + 1) : src
          ...
          return (
            <div
              key={i}
              className={[
                "bg-card rounded-xl border border-border border-l-[3px]",
                cfg.leftBorder,
            ...
            >
              {/* Standard name + score */}
              <div className="flex items-center justify-between gap-2">
                <span className="text-[15px] font-bold tracking-tight">{item.standard}</span>
                {item.score != null && (
                  <span className={`text-sm font-semibold tabular-nums ${cfg.scoreCls}`}>
              ...
              {/* One-line reason */}
              <p className="text-xs text-muted-foreground leading-relaxed truncate">{reason}</p>
            </div>
          )
        })}
      </div>
